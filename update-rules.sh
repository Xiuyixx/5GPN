...
python3 - "${DNSDIST_TEMPLATE}" "${GFWLIST_LUA}" "${CHINALIST_LUA}" "${SERVER_IP}" "${CERT_BASENAME}" "${OVERSEAS_PRIVATE_DNS_SERVERS}" "${OVERSEAS_PUBLIC_DNS_SERVERS}" "${DNSDIST_CONF}" <<'PYEOF'
import sys
