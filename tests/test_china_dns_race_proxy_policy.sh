...
if [[ "${install}" != *'for svc in dnsdist sniproxy quic-proxy china-dns-race-proxy proxy-gateway-ios-profile'* ]]; then
...
if [[ "${install}" != *'rm -f /etc/systemd/system/{sniproxy,quic-proxy,china-dns-race-proxy,proxy-gateway-ios-profile,update-dnsdist-rules}.*'* ]]; then
